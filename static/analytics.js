(function (global) {
  ...
  const TERMINAL_JOBS = new Set(["completed", "completed_with_errors", "failed", "canceled"]);
  const sentTerminalJobs = new Set();
  const replayFinishedSessions = new Set();
  const jobContext = new Map();
  const portfolioCache = new Map();
  const observedDrawingManagers = new WeakSet();
  ...
  function sanitizeValue(value, depth) {
    ...
    if (Array.isArray(value)) {
      return value.slice(0, 12).map((v) => sanitizeValue(v, depth + 1)).filter((v) => v !== undefined);
    }
  ...
  function trackVirtualPage(name) {
    if (!name || currentVirtualPage === name) return false;
    ...
    safeCall(() => {
      new URL(global.location.href).searchParams.forEach((value, key) => {
        if (SAFE_QUERY.has(key) && !SENSITIVE_QUERY.test(key)) url.searchParams.set(key, value.slice(0, 120));
      });
    });
    url.hash = name;
  ...
  function loadMetrika() {
    ...
    script.onerror = function () { /* analytics is intentionally best-effort */ };
  ...
  function strategyName(strategyId) {
    return safeCall(() => global.STRATEGIES && global.STRATEGIES[strategyId] && global.STRATEGIES[strategyId].name) || undefined;
  }
  ...
  function normalizeAssignments(raw) {
    ...
    Object.keys(raw).forEach((ticker) => {
      ...
      rows.forEach((row) => {
        ...
        out.set(ticker + "::" + row.strategy_id, {
          ticker: ticker,
  ...
  function normalizeInstruments(raw) {
    ...
    raw.forEach((item) => {
      const ticker = typeof item === "string" ? item : item && item.ticker;
      if (ticker) out.add(String(ticker));
    });
    return out;
  }
  ...
  function rememberPortfolio(portfolio) {
    ...
    portfolioCache.set(String(portfolio.id), {
      strategies: normalizeAssignments(portfolio.ticker_strategies),
      instruments: normalizeInstruments(portfolio.instruments)
    });
  }

  function strategyContext(portfolioId, tickers) {
    const cached = portfolioCache.get(String(portfolioId));
    if (!cached) return { rows: [], ids: [], count: undefined };
    const allowed = new Set(Array.isArray(tickers) ? tickers : []);
    const rows = [];
    cached.strategies.forEach((row) => {
      if (!allowed.size || allowed.has(row.ticker)) rows.push(row);
    });
    const ids = Array.from(new Set(rows.map((row) => row.strategy_id))).slice(0, 12);
    return { rows: rows, ids: ids, count: ids.length };
  }

  function handlePortfolioSave(portfolioId, body, responseData) {
    const cached = portfolioCache.get(String(portfolioId));
    const nextInstruments = normalizeInstruments(body && body.instruments);
    if (cached && nextInstruments.size) {
      cached.instruments.forEach((ticker) => {
    ...
    if (responseData && responseData.id) rememberPortfolio(responseData);
    else if (cached) cached.instruments = nextInstruments;
  }

  function handlePortfolioStrategySave(portfolioId, body, responseData) {
    const next = normalizeAssignments(body && body.ticker_strategies);
    const cached = portfolioCache.get(String(portfolioId));
    const prev = cached ? cached.strategies : new Map();
    ...
    next.forEach((row, key) => {
      ...
      } else if (JSON.stringify(old.parameters || {}) !== JSON.stringify(row.parameters || {})) {
    ...
    if (responseData && responseData.id) rememberPortfolio(responseData);
    else if (cached) cached.strategies = next;
  ...
  function handleJob(jobId, job) {
    ...
    if (!ctx || !TERMINAL_JOBS.has(job.status)) return;
  ...
  function marketDataParams(url, body, status) {
    const pathTicker = safeCall(() => decodeURIComponent((url.pathname.match(/\/instruments\/([^/]+)\//) || [])[1] || "")) || undefined;
    return {
  ...
  function installFetchObserver() {
    ...
    const wrapped = async function (input, options) {
      ...
      safeCall(() => {
        ...
        clone.json().then((data) => {
          if (method === "GET" && path === "/api/portfolios" && response.ok && Array.isArray(data)) data.forEach(rememberPortfolio);
          ...
          const portfolioPut = path.match(/^\/api\/portfolios\/([^/]+)$/);
          if (method === "PUT" && portfolioPut && response.ok) handlePortfolioSave(portfolioPut[1], body || {}, data);

          const strategyPatch = path.match(/^\/api\/portfolios\/([^/]+)\/strategies$/);
          if (method === "PATCH" && strategyPatch && response.ok) handlePortfolioStrategySave(strategyPatch[1], body || {}, data);

          const strategyPreset = path.match(/^\/api\/strategy-presets\/([^/]+)$/);
          if (method === "PUT" && strategyPreset && response.ok) {
            trackGoal("strategy_settings_changed", {
              strategy_id: strategyPreset[1],
              strategy_name: strategyName(strategyPreset[1]),
              module: "strategy_library"
            });
          }

          if (method === "DELETE") {
            const instrumentDelete = path.match(/^\/api\/portfolios\/[^/]+\/instruments\/([^/]+)$/);
            if (instrumentDelete && response.ok) trackGoal("portfolio_instrument_removed", { ticker: decodeURIComponent(instrumentDelete[1]) });
          }
          ...
          if (method === "GET" && /^\/api\/backtests\/[^/]+$/.test(path) && response.ok) trackGoal("backtest_result_opened", {});
          ...
          if (!response.ok && /\/api\/(?:candles|market-data|securities|portfolios\/[^/]+\/instruments\/[^/]+\/download-data)/.test(path)) {
            trackGoal("market_data_load_failed", marketDataParams(url, body, response.status));
          }
        }).catch(function () { /* non-JSON response: business request still owns its own error handling */ });
      });
      return response;
    };
    ...
    if (enabled) {
      originalFetch("/api/portfolios", { credentials: "same-origin" })
        .then((response) => response.ok ? response.json() : null)
        .then((data) => { if (Array.isArray(data)) data.forEach(rememberPortfolio); })
        .catch(function () { /* cache enrichment only; never affect the app */ });
    }
  }
  ...
  function observeDrawingManager(tile) {
    const manager = tile && tile.drawingMgr;
    if (!manager || observedDrawingManagers.has(manager) || typeof manager.onChange !== "function") return;
    observedDrawingManagers.add(manager);
    manager.onChange(function (mgr, detail) {
      if (!detail || !detail.created) return;
      const drawing = Array.isArray(mgr.drawings) ? mgr.drawings.find((item) => item.id === detail.created) : null;
  ...
  function installReplayHooks() {
    ...
    if (!page || wrappedPages.has(page)) return;
    wrappedPages.add(page);
    wrapMethod(page, "_togglePlay", function (args, before) {
      if (!before.playing || this.playing || !this.state || this.state.finished) return;
      const now = Date.now();
      const previousPause = pauseDedup.get("replay_pause") || 0;
      if (now - previousPause <= 1200) return;
  ...
  function installUiObserver() {
    document.addEventListener("click", function (event) {
      ...
      const tab = target.closest && target.closest("[data-tab]");
      if (tab && tab.dataset.tab) {
        const changed = trackVirtualPage(tab.dataset.tab);
        if (changed && tab.dataset.tab === "charts") trackGoal("chart_analysis_opened", {});
        if (changed && tab.dataset.tab === "replay") trackGoal("market_replay_opened", {});
      }

      if (target.closest && target.closest("#openAssignModalBtn")) {
        const modal = document.getElementById("strategyAssignModal");
        if (!modal || modal.classList.contains("hidden")) trackGoal("portfolio_strategies_opened", {});
      }
      ...
      if (configure) {
        ...
        if (!openPanel) {
          trackGoal("strategy_settings_opened", {
            ...
            strategy_name: strategyName(configure.dataset.strategyId),
            module: "portfolio_strategies"
          });
        }
      }

      const libraryHeader = target.closest && target.closest("[data-toggle-strategy]");
      if (libraryHeader) {
        const card = libraryHeader.closest("[data-strategy-card]");
        if (!card || !card.classList.contains("open")) {
          trackGoal("strategy_settings_opened", {
            strategy_id: libraryHeader.dataset.toggleStrategy,
            strategy_name: strategyName(libraryHeader.dataset.toggleStrategy),
            module: "strategy_library"
          });
        }
      }
    ...
    document.addEventListener("change", function (event) {
      ...
      if (!element || !element.matches || !element.matches("#gtIndicatorsPop input[data-ind]") || !element.checked) return;
      const page = global.ChartAnalysisPage;
      const tile = page && page.activeTile;
      const exists = tile && tile.indicatorMgr && tile.indicatorMgr.list().some((item) => item.type === element.dataset.ind);
      if (exists) trackGoal("chart_indicator_added", { ticker: tile.symbol, indicator: element.dataset.ind });
    }, false);
  }
