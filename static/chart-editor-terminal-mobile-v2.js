...
(function (g) {
   ...
   * (compare/add-symbol, an unlabeled circle-plus, and one glyph that
   * rendered as the bare letter "Д" - not identifiable from the reference
   * screenshot as any feature this app has) ahead of the chart-type/
   * indicators icons. Per explicit direction, matched all of them visually
   * even though this app has no compare-symbol or account-tier concept:
   * the two with a real equivalent feature here (Шаблоны, Оповещения)
   * proxy-click the existing hidden desktop buttons - reusing their
   * already-relocated popovers (relocatePopovers below /
   * chart-editor-terminal-compat.js) instead of duplicating that plumbing -
   * and the menu icon proxies to the existing "Ещё" button, the closest
   * thing to a catch-all drawer this page has. The "Д" glyph and the
   * chart-type icon (a CSS background-image on the real #gtChartType
   * select, see mobileCSS) have nothing to proxy to, so they're inert. */
  ...
  function sync() {
    ...
    lastPhone = isPhone; buildRail(); refreshRail(); buildBottomControls(); closeButton(); compactTickerLabels(); wireTickerTap(); buildTopbarExtras(); relocatePopovers(); renderMarketHeader();
  ...
  /* Real TradingView's top row has no separate timeframe pill (see
   * Tradingview.PNG) - interval switching lives inside the "..." menu
   * instead. #gtTimeframe (hidden on phone, mobileCSS above) stays the
   * source of truth other code reads/writes (_wireGlobalToolbar,
   * _refreshGlobalHeader keep its .value in sync), this just adds a phone-
   * only picker group into the existing "Ещё" popover that drives it
   * through the same _commandSetTimeframe the hidden select's onchange
   * already used. */
  const oldMore = Page._renderMorePopover;
  Page._renderMorePopover = function () {
    const result = oldMore.apply(this, arguments);
    if (!phone()) return result;
    const pop = this.root && this.root.querySelector("#gtMorePop");
    const tile = this.activeTile;
    if (!pop || !tile) return result;
    pop.insertAdjacentHTML("afterbegin", `
      <div class="ca-more-group">
        <div class="ca-more-heading">Таймфрейм</div>
        <div class="ca-range-presets">
          ${tile.listTimeframes().map((t) => `<button class="range-preset ${t.id === tile.timeframe ? "active" : ""}" data-sl-tf="${esc(t.id)}">${esc(t.label)}</button>`).join("")}
        </div>
      </div>
      <div class="ca-more-sep"></div>
    `);
    pop.querySelectorAll("[data-sl-tf]").forEach((b) => (b.onclick = () => {
      pop.classList.add("hidden");
      this._commandSetTimeframe(b.dataset.slTf);
    }));
    return result;
  };
