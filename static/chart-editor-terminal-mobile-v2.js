...
(function (g) {
   ...
   * single slim action row here, not two. Two of the five original buttons
   * were pure duplicates: "Диапазон" only ever forwarded to the same "..."
   * menu already reachable from the top toolbar, and "Детали инструмента"
   * opened the exact same watchlist drawer as "Список котировок" (just
   * additionally pre-selecting the active symbol, which the drawer already
   * does once you tap any row). The scale toggles (%/лог/авто) now live
   * behind a gear popover instead of sitting bare in the row, matching how
   * real TradingView tucks them away rather than showing them inline. */
  function buildBottomControls() {
    ...
    center.insertAdjacentHTML("beforeend", `<div class="sl-chart-controls"><button type="button" data-sl-range>Диапазон ▾</button><span class="sl-chart-controls-sep"></span><span class="sl-chart-controls-clock" data-sl-clock>--:--:-- (UTC+3)</span><span class="sl-chart-controls-sep"></span><button type="button" class="sl-chart-controls-icon" data-sl-scale title="Настройки шкалы" aria-label="Настройки шкалы">${icon("gear")}</button><button type="button" class="sl-chart-controls-icon" data-sl-quotes title="Список инструментов" aria-label="Список инструментов">${icon("list")}</button><div class="sl-scale-pop hidden" role="menu"><button type="button" data-sl-percent>%</button><button type="button" data-sl-log>Лог</button><button type="button" data-sl-auto>Авто</button></div></div>`);
    const controls = center.querySelector(".sl-chart-controls"), pop = controls.querySelector(".sl-scale-pop");
    controls.querySelector("[data-sl-range]").onclick = () => Page.root.querySelector("#gtMoreBtn")?.click();
    controls.querySelector("[data-sl-scale]").onclick = (event) => {
      event.stopPropagation(); const opening = pop.classList.contains("hidden"); pop.classList.toggle("hidden");
      if (opening) document.addEventListener("click", () => pop.classList.add("hidden"), { once:true, capture:true });
    };
    const applyMode = (mode) => { const core = Page.activeTile?.core; if (!core) return; let current = 0; try { current = core.chart.priceScale("right").options().mode || 0; } catch (_) {} const next = current === mode ? 0 : mode; try { core.chart.priceScale("right").applyOptions({ mode:next }); } catch (_) {} pop.querySelector("[data-sl-percent]").classList.toggle("active", next === 2); pop.querySelector("[data-sl-log]").classList.toggle("active", next === 1); };
    pop.querySelector("[data-sl-percent]").onclick = () => applyMode(2); pop.querySelector("[data-sl-log]").onclick = () => applyMode(1);
    pop.querySelector("[data-sl-auto]").onclick = () => Page.activeTile?.core?.chart.priceScale("right").applyOptions({ autoScale:true });
    controls.querySelector("[data-sl-quotes]").onclick = () => { Page.watchlist?.openMobileDrawer(); requestAnimationFrame(() => Page.watchlist?.setActive(Page.activeTile?.symbol)); };
