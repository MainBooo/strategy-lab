...
(function (g) {
   ...
   * single slim action row here, not two, and two of the five original
   * buttons were pure duplicates: "Диапазон" only ever forwarded to the
   * same "..." menu already reachable from the top toolbar, and "Детали
   * инструмента" opened the exact same watchlist drawer as "Список
   * котировок" (just additionally pre-selecting the active symbol, which
   * the drawer already does once you tap any row). The clock is dropped
   * outright - the phone's own status bar already shows the time. */
  function buildBottomControls() {
    ...
    center.insertAdjacentHTML("beforeend", `<div class="sl-chart-controls"><button type="button" data-sl-range>Диапазон ▾</button><span class="sl-chart-controls-sep"></span><button type="button" data-sl-percent>%</button><button type="button" data-sl-log>лог</button><button type="button" data-sl-auto>авто</button><span class="sl-chart-controls-sep"></span><button type="button" class="sl-chart-controls-icon" data-sl-quotes title="Список инструментов" aria-label="Список инструментов">${icon("list")}</button></div>`);
    ...
    center.querySelector("[data-sl-quotes]").onclick = () => { Page.watchlist?.openMobileDrawer(); requestAnimationFrame(() => Page.watchlist?.setActive(Page.activeTile?.symbol)); };
  }
