...
(function (g) {
  ...
  const icon = (name) => ({
    ...
    zoom:'<svg viewBox="0 0 24 24"><circle cx="10" cy="10" r="6"/><path d="M10 7v6M7 10h6M20 20l-4.5-4.5"/></svg>',
    compare:'<svg viewBox="0 0 24 24"><rect x="3" y="3" width="13" height="13" rx="2"/><rect x="8" y="8" width="13" height="13" rx="2"/></svg>'
  }[name] || "");
  ...
  const groups = [
    ...
    ["zoom","Увеличение области",icon("zoom"),[["zoom_area","Увеличение области"]]],
  ...
  function buildTopbarExtras() {
    ...
    tf.insertAdjacentHTML("afterend", `<button class="icon-btn" id="slTvTemplatesBtn" type="button" title="Шаблоны" aria-label="Шаблоны">${icon("shieldPlus")}</button><button class="icon-btn" id="slTvAlertsBtn" type="button" title="Оповещения" aria-label="Оповещения">${icon("circlePlus")}</button><button class="icon-btn" id="slTvCompareBtn" type="button" title="Сравнение" aria-label="Сравнение / наложение символа">${icon("compare")}</button><div class="gt-menu sl-tv-interval-menu"><button class="icon-btn sl-tv-interval" id="slTvIntervalBtn" type="button" title="Таймфрейм" aria-label="Таймфрейм" aria-haspopup="true">${esc(compactTf(tf.value))}</button><div class="ca-popover hidden" id="slTvIntervalPop"></div></div>`);
    ...
    proxyClick("slTvCompareBtn", "gtCompareBtn");
