...
| Compare/add symbol | наложение второго тикера на график | `ChartTile.addCompareSymbol()` — до 6 наложенных %-change оверлеев, живой пересчёт базы при пане/зуме, легенда+попап поиска (десктоп/мобильный) | да | **PARITY (испр. эта сессия)** | живой Playwright на проде реальными кликами — добавление через попап/поиск, удаление через легенду, self-compare guard, ре-базирование при сужении диапазона, ре-загрузка при смене таймфрейма/символа, все подтверждены программно и визуально (скриншот) |
...
| Zoom tool (area-zoom, отдельно от pinch) | | новый `zoom_area` tool (ephemeral, тот же паттерн что `measure`) — тащит бокс, на отпускании зумит временную ось ровно на диапазон бокса (`chart.timeScale().setVisibleRange`, без 15%-паджинга `core.setVisibleRange`); ценовая ось остаётся на текущем autoScale и сама подстраивается под свечи в новом окне, а не под явную цену бокса — сознательно не заводили второе "закреплённое" состояние ценовой шкалы, которое конфликтовало бы с уже существующей кнопкой «Автомасштаб»/`scaleAuto` | да | **PARITY (испр. эта сессия)** | живой Playwright на проде — реальный `page.mouse` drag (не программный `dm.addDrawing`): видимый диапазон после отпускания побайтово совпал с временами обеих стартовых координат (без округления/паджинга), второй drag сразу зумит повторно без переселекции инструмента, `drawings.length` остаётся 0 (инструмент вообще не пишет в БД — эфемерный, как `measure`), дашед-бокс виден на скриншоте mid-drag, консоль чистая |
...
- **Zoom tool (area-zoom)** — из MISSING в PARITY. Единственный оставшийся
  самодостаточный (без нового backend) MISSING-пункт линейки инструментов;
  соседний Compare/add symbol остался MISSING (нужен multi-symbol overlay
  backend, отдельная задача). Новый `zoom_area` tool — тот же `ephemeral`
  паттерн, что `measure` (см. `def.ephemeral` в `_finishDraft`): драг
  рисует дашед-бокс-превью (`kind:"zoom_area"`, без price/date-подписей —
  это навигационный жест, не измерение), на отпускании
  `_applyAreaZoom()` зумит временную ось ровно на `[min(t1,t2),
  max(t1,t2)]` через `chart.timeScale().setVisibleRange()` без паджинга
  (в отличие от `core.setVisibleRange()`, добавляющего 15% отступа) и
  сразу переармирует тот же tool. Сознательно НЕ трогает ценовую ось —
  она остаётся на своём текущем autoScale и естественно подстраивается
  под свечи, попавшие в новое временное окно; альтернатива (жёстко
  закрепить ценовой диапазон бокса через `autoscaleInfoProvider` +
  `autoScale:false`) потребовала бы отдельного состояния "залочен ли
  ценовой диапазон", которое конфликтовало бы с уже существующей кнопкой
  «Автомасштаб»/`act === "scaleAuto"` (`chart-analysis.js`) — решили не
  вводить второй, конкурирующий механизм сброса.
  **Верифицировано** вживую на проде через РЕАЛЬНЫЕ `page.mouse`
  down/move/up (не программный `dm.addDrawing()` — специально, чтобы
  проверить именно pointer-путь): видимый диапазон после отпускания
  побайтово совпал с временами, вычисленными из тех же самых экранных
  координат до драга (`{from:1787088960,to:1787114940}` — точное
  совпадение, без округления/паджинга); второй drag сразу зумит повторно
  без переселекции инструмента из рейла; `drawings.length` остаётся 0 на
  каждом шаге — инструмент вообще ничего не пишет в БД (эфемерный, как
  `measure`), поэтому и нечего было удалять после проверки; дашед-бокс
  подтверждён на скриншоте mid-drag (и как тонкая линия при
  горизонтальном драге, и как полноценный прямоугольник при 2D-драге);
  консоль чистая (только пред-существующие 404 `favicon.ico`/`/api/me`,
  не связаны с фичей). Кнопка добавлена в оба рейла — мобильный (новая
  однопунктовая группа `zoom` с иконкой лупы, `chart-editor-terminal-
  mobile-v2.js`) и десктопный (вероятно мёртвый, но синхронизируемый по
  традиции этой серии сессий `TOOL_BUTTONS`, `chart-analysis.js`). 198
  pytest + оба JS runtime suites зелёные (allTools +1 имя; `zoom_area`
  заведён в тот же ephemeral-эксклюжн из `persistentTools`, что `measure`;
  dedicated-тест на живой preview mid-drag, точный `setVisibleRange(from,
  to)` без паджинга на прямом и реверсном драге, re-arm, и no-op на
  вырожденном нулевом боксе — пришлось сравнивать поля `.from`/`.to`
  по отдельности, не `deepStrictEqual` на весь объект: `{from,to}`
  строится внутри vm-песочницы теста, другой JS-realm, чем объектный
  литерал самого теста — тот же cross-realm урок, что уже был найден в
  `duplicateSelection`-тесте Multiselect/Grouping).
- **Compare/add symbol** — из MISSING в PARITY. Единственный оставшийся
  крупный пункт линейки инструментов ТЗ. Матрица годами держала это как
  «нет back-end для multi-symbol overlay» — расследование этой сессии
  показало, что это было неверно: `/api/candles` (`app.py:api_candles`)
  уже принимает произвольный `symbol`, единственная проверка —
  `CHART_ANALYSIS_ACCESS`, тот же флаг, что и у основного графика; никакого
  нового backend-эндпоинта не понадобилось, только второй потребитель уже
  существующего generic REST API. Новый `ChartTile.addCompareSymbol()` —
  до `COMPARE_MAX=6` наложенных линий, каждая на общей `left` price scale
  (включается/выключается вместе с первым/последним оверлеем через
  `chart.priceScale("left").applyOptions({visible})` — не трогает основную
  `right`-шкалу цены свечей), значения — %-change от базовой свечи, а не
  сырая цена (иначе разномасштабные символы, напр. BTC против мем-коина,
  были бы визуально бессмысленны на одной шкале). **Живое ре-базирование**
  на пане/зуме — определяющая часть настоящего TradingView Compare-режима
  (без него сравнение теряет смысл после скролла): подписка на
  `timeScale().subscribeVisibleLogicalRangeChange`, коалесцируется через
  `requestAnimationFrame` (иначе `series.setData()` — дорогой вызов —
  дёргался бы на каждый промежуточный кадр драга), база — первая свеча
  оверлея на/после левого края видимого диапазона (честный фоллбэк на
  последнюю свечу, если история оверлея не доходит так далеко). Тот же
  «живой пересчёт из core.candles» принцип, что уже используют
  `anchored_vwap`/`cyclic_lines`/`regression_trend`, но впервые применённый
  не к DrawingManager-примитиву, а к настоящей второй lightweight-charts
  `LineSeries` (`LWC.LineSeries`, `priceFormat:{type:"custom"}` — свой
  форматтер `+N.NN%`). Живой tail-refresh (та же схема, что
  `ChartCore.refreshTail`/`mergeTailCandles`, реализована отдельно против
  простого массива `entry.candles`, т.к. оверлею не нужно ничего из
  остального, что тащит `ChartCore` — drawings/индикаторы/live-tick)
  подключён в существующий `_refreshTail()`-тик (каждые 25с), падение
  одного оверлея не роняет обновление основной плитки. Смена
  таймфрейма/символа тайла (`setTimeframe`/`_setSymbol`) перезагружает
  каждый оверлей на новом таймфрейме (`_reloadCompareSymbols`); смена
  символа на уже сравниваемый — сначала снимает этот оверлей (сравнивать
  символ с самим собой бессмысленно). UI — новая кнопка «⧉» в обоих
  рейлах (десктоп `gtCompareMenu`/`gtCompareBtn`+попап с поиском через уже
  существующий `/api/market-data/instruments`; мобильный — 4-я
  proxy-иконка `slTvCompareBtn`→`gtCompareBtn`, по образцу уже
  существовавших Шаблоны/Оповещения) — своя лёгкая типология поиска, не
  переиспользование `WatchlistSidebar` (тот заточен под single-select
  «заменить основной символ», не «добавить второй поверх», репурпоз был
  бы рискованнее нового самодостаточного попапа. Легенда (`sl-chart-
  legend`, `chart-editor-terminal-compat.js`) получила чипы для оверлеев
  — тот же визуальный язык, что у индикаторов (цветная точка+имя+значение+
  toggle+удалить), без шестерёнки настроек (нечего настраивать у
  %-оверлея кроме видимости).
  **Найден и исправлен реальный баг** при живой проверке (не в реализации
  из плана, а в её первой версии): удаление оверлея через чип легенды не
  вызывало `_saveWorkspaceState()` (в отличие от попапа, где вызов уже
  был) — символ переживал удаление в `localStorage`, и хуже: `_loadOrInit()`
  переприменяет `_initialCompareSymbols` (снятое один раз в конструкторе)
  на КАЖДУЮ смену символа тайла в течение всей его жизни, не только при
  первом монтировании (тот же паттерн, что уже негласно существует для
  индикаторов — не трогал, вне scope) — из-за этого удалённый вживую
  оверлей воскресал при следующей смене основного символа тайла, даже без
  перезагрузки страницы. Исправлено синхронизацией `_initialCompareSymbols`
  с реальным состоянием на каждый `_notifyCompareChanged()`, плюс
  добавлен пропущенный `_saveWorkspaceState()` в оба обработчика чипа
  легенды. Воспроизведено и подтверждено исправленным вживую на проде.
  **Верифицировано** вживую на проде (QA-логин): ETHUSDT добавлен как
  оверлей на SOLUSDT (и через попап с реальным поиском/кликом, и
  программно) — 1200 свечей, `left`-шкала стала видимой, легенда показала
  чип с живым %; скриншот подтвердил узнаваемую TradingView-картину
  (оранжевая %-линия поверх свечей, отдельная % шкала слева, основная
  цена справа); сужение видимого диапазона подтверждённо пересчитало %
  (0.248%→0.176% на той же паре свечей, разный левый край); удаление
  через чип легенды реальным кликом — `left`-шкала скрылась, легенда
  пуста; self-compare guard (`addCompareSymbol(tile.symbol)` → `null`) и
  guard на смену основного символа на уже сравниваемый (оверлей снимается)
  оба подтверждены программно; `_refreshCompareTails()` вызван напрямую —
  не упал, пересчитал %; мобильная proxy-кнопка присутствует и видима
  (32×44px) на 390×844; консоль чистая на каждом шаге. 198 pytest + оба JS
  runtime suites зелёные (без изменений — фича не тронула ни один файл,
  который они покрывают; `chart-tile.js`/`chart-analysis.js` проверялись
  только `node --check` + живым Playwright, у ChartTile нет unit-test
  гарнесса — потребовал бы мокать ResizeObserver/lightweight-charts с
  нуля, непропорционально этой задаче, когда живая проверка на реальном
  lightweight-charts v5 и так более достоверна, чем мок).
- **XABCD harmonic-pattern авто-классификация** (`classifyXabcdPattern`,
  `drawings.js`) — Gartley/Bat/Butterfly/Crab по канонической ratio-
  таблице Скотта Карни (широко задокументированные, не гаданные —
  четыре паттерна широко публикуются во множестве независимых
  источников), в отличие от Inside Pitchfork/Gann Square-Box, чьи
  формулы это ТЗ сознательно НЕ реализует без уверенного источника.
  Только `xabcd_pattern` (5 анкоров, есть X) — единственный тул этой
  labeled-zigzag семьи с полным набором ног (XA/AB/BC/CD/AD) для теста;
  ABCD/Three Drives/Elliott остаются без классификации, у них нет
  X-анкора. AD/XA — основной дискриминатор (диапазоны 4 паттернов почти
  не пересекаются: ~0.72-0.86 / 0.82-0.93 / 1.15-1.75 / 1.50-1.75),
  AB/XA и CD/BC сужают дальше; все диапазоны сознательно расширены
  относительно точных канонических чисел (0.618/0.786/... — реальный
  рыночный свинг почти никогда не попадает ровно в них) — это
  принципиально приближённый классификатор, как любой harmonic-scanner,
  не точное измерение. Направленность (бычий/медвежий) — по знаку
  XA-ноги. Рендер — подпись `«<Паттерн> (бычий/медвежий)»` у точки D,
  выше её собственной буквенной метки, чтобы не пересекались.
  **Верифицировано** вживую на проде: Gartley нарисован на реальных
  барах SOLUSDT с точными целевыми ratio (61.8%/140.0%→AD 79%),
  подпись «Гартли (бычий)» подтверждена на скриншоте у точки D,
  hit-test подтверждён, `drawings.length===0` после удаления+reload —
  не осталось мусора в БД прода, консоль чистая. Юнит-тест
  (`classifyXabcdPattern` экспортирован через `CE.Drawings` для прямого
  тестирования) — точные алгебраически решённые ratio-цели для Gartley
  (прямой X<A бычий + зеркальный X>A медвежий, тот же |ratio|) и Bat;
  no-match кейс (ratio вне всех четырёх диапазонов → `null`, не
  форсированное совпадение); degenerate нулевая XA-нога → `null`, не
  деление на ноль. 198 pytest + оба JS runtime suites зелёные.
- **Технический долг закрыт**: desktop `chart-analysis.js` `TOOL_BUTTONS`
  массив (~60 записей, дублировавший рейл на протяжении всей этой серии
  сессий — см. "вероятно мёртвый код" в changelog частей 5/10/12/13/14/17)
  окончательно подтверждён мёртвым и удалён вместе со всей его
  инфраструктурой (`#caTools`-шаблон, `_wireToolTools()`/`#caDeleteBtn`,
  мёртвая DOM-синхронизация active-состояния в `_setActiveTile`, CSS
  `.ca-tool-btn`/`.ca-tool-danger`). Живая проверка на проде подтвердила:
  `chart-editor-terminal-mobile-v2.js`'s `buildRail()` безусловно (не
  только на телефоне) перезаписывает `#caTools.innerHTML` на грубый
  `.tv-rail`-разметку сразу после первого `_build()` — `.ca-tool-btn`/
  `#caDeleteBtn` не существуют в DOM ни на одном viewport ни на одном
  этапе жизни страницы, кроме теоретического одного animation-frame'а до
  перезаписи. Реальная кнопка удаления выделенного объекта — Floating
  Toolbar (`ChartTile._renderFloatToolbar`) плюс клавиша Delete, обе уже
  работают независимо. Реальный mouse-драг trend_line/select/delete
  round-trip подтверждён на десктопе и мобильном viewport, консоль
  чистая, скриншоты визуально идентичны состоянию до чистки (чистка не
  меняла ни одного видимого пикселя — только удаляла код, который
  никогда не рендерился). Один pytest потребовал правки (контракт-тест
  на consistency "circle"→"Эллипс" по трём файлам проверял и
  chart-analysis.js — убрана только эта часть ассерта, остальные два
  файла (drawings.js/rail) остаются реальным источником истины и
  по-прежнему проверяются).

## Часть 24 (2026-08-19) — критический баг первого впечатления найден и исправлен

Пользователь спросил прямо: «открою страницу и не смогу отличить на
TradingView я или в StrategyLab?». Функциональный объём ТЗ (178 пунктов)
был исчерпан в части 22-23 (см. changelog выше), поэтому эта сессия
вместо очередного drawing-инструмента сделала то, что ни разу не
делалось за 23 предыдущие части: реальную сквозную UX-проверку
**обычным** пользовательским сценарием (настоящий клик по вкладке,
настоящий F5) на десктопном и мобильном viewport, со сравнением
скриншотов против `Tradingview.PNG`.

- **Реальный клик по вкладке «Анализ графиков»** — работает: график
  поднимается сам, `_built:true`, 7 canvas. Опровергает часть тревоги из
  частей 17-20 — тот прод-баг был именно про **программную**
  Playwright-загрузку с уже сохранённым в localStorage табом, реальный
  клик от него не страдает.
- **Визуальное сравнение** (десктоп 1440×900 и мобайл 390×844 против
  референса) — очень близко: та же композиция панелей (toolbar сверху,
  рейл инструментов слева, price-scale справа, времена снизу), тот же
  фирменный логотип TradingView внизу графика (это не подделка — реальный
  watermark библиотеки `lightweight-charts`, той же, на которой построен
  сам TradingView, поэтому он идентичен на обеих картинках), тёмная
  палитра/типографика совпадают на глаз.
- **НАЙДЕН РЕАЛЬНЫЙ PROD-БАГ, не тестовый артефакт** (опровергает
  предположение частей 17-20 «не бага продукта»): обычный **F5 на
  вкладке «Анализ графиков»** оставляет полностью пустой экран —
  вкладка подсвечена активной, но график не рендерится
  (`hasCanvas:0, built:false, tiles:0`). Скриншот подтвердил визуально:
  под активной вкладкой буквально ничего, кроме фона. Любой реальный
  пользователь, который зашёл на страницу, открыл «Анализ графиков» (это
  наверняка — основная фича), затем обновил страницу браузером — увидит
  этот пустой экран. Это ровно тот сценарий, который мгновенно выдаёт
  «это не настоящий TradingView».
  **Root cause**: `templates/index.html` грузит `app.js` (строка 315)
  раньше `chart-analysis.js`/`market-replay.js` (строки 322/320) — как
  обычные блокирующие `<script>` без `defer`, они выполняются строго по
  порядку появления в HTML. `app.js` вызывает `bootstrap()`→`initTabs()`
  синхронно в конце своего же исполнения — то есть до того, как браузер
  вообще начал парсить/грузить `chart-analysis.js`. `activateTab("charts")`
  из `initTabs()` (если это сохранённая с прошлого визита вкладка — а она
  сохраняется в `localStorage` при каждом переключении) делал
  `if(name==="charts"&&window.ChartAnalysisPage)…` — на этот момент
  `window.ChartAnalysisPage` ещё не существует, условие ложно, `.init()`
  просто тихо не вызывается, а вкладка уже помечена активной. Тот же
  класс бага у `MarketReplayPage`/вкладки Market Replay.
  **Исправлено** (`4eee647`, `static/app.js`): `activateTab` для
  charts/replay теперь вызывает `initTabPageWhenReady()` — polling через
  `requestAnimationFrame` (до 60 кадров) до появления нужного `window.*`
  глобала, с проверкой, что пользователь тем временем не переключился на
  другую вкладку (иначе поздний `.init()` мог бы дорисовать график поверх
  уже открытой другой вкладки). Верифицировано вживую на проде: двойной
  подряд F5 на charts-вкладке — оба раза `hasCanvas:7, built:true`,
  консоль чистая (только предсуществующий favicon 404 и deprecated-meta
  warning, не связаны). 198 pytest + оба JS runtime suites зелёные без
  единой правки тестов — баг был чисто в `app.js`, drawing-движок не
  тронут.
...
- **Индикаторные панели** — проверено в части 26 (см. ниже): crosshair
  sync и resize между panes работают нативно (lightweight-charts' own
  multi-pane API), drawing tools внутри oscillator pane были реальным
  багом (не просто пробелом) — теперь заблокированы вместо тихого
  создания повреждённых объектов; полная поддержка рисования *внутри*
  oscillator pane (своя price-scale, свой render-слой) не реализована —
  это отдельный, значительно больший кусок работы.
- **Keyboard shortcuts** сверх Ctrl+Z/Shift+Z/Delete/Escape/Ctrl+G/Ctrl+D
  — не сверялись построчно со списком ТЗ §104.

## Часть 25 (2026-08-19) — Delete/Ctrl+Z работали только после клика по canvas, не по наведению

Живая проверка реального workflow (не только programmatic `dm.addDrawing`)
нашла реальный баг: `DrawingManager._onKeyDown` был навешан на сам
контейнер пейна (`el.addEventListener("keydown", ...)`), а не на
`document`. Keydown-события следуют за фокусом клавиатуры, а не за
позицией мыши — так что этот listener фактически срабатывал только когда
контейнер уже имел DOM-фокус (что происходит после реального клика по
canvas), а собственная проверка `_onKeyDown` (`!_pointerInside &&
activeElement !== container`) явно расчитывала, что наведения мыши
достаточно — как в настоящем TradingView.

**Как воспроизвели**: нарисовали объект реальным mouse-драгом (фокус
уходит на контейнер — Delete/Ctrl+Z работали); затем выбрали тот же
объект НЕ через canvas (`dm.select(id)` — эквивалент клика по строке
Object Tree, которая является `<div tabIndex="-1">` и не получает фокус
по клику), навели мышь на график без клика — Delete/Ctrl+Z ничего не
делали, хотя объект оставался выделен и подсвечен. Любой workflow, где
выбор происходит не через canvas (Object Tree, смена тикера, любая
панель), молча ломал все keyboard-хоткеи до следующего клика по графику.

**Исправлено** (`3f0d60f`): listener перенесён на `document` — теперь
`_pointerInside` (уже существовавший флаг) сам решает, актуален ли
хоткей для этого пейна, ровно как задумывалось изначально. Поскольку
document-level listener видит вообще все keydown на странице, добавлен
guard на реальные поля ввода (input/textarea/select/contenteditable) —
иначе текстовое поле в другом месте страницы (поиск тикера, цена алерта)
проигрывало бы рисовательным хоткеям, если мышь просто лежит над
графиком во время печати.

**Верифицировано** вживую на проде (реальные mouse-клики/драги, не
только programmatic): Delete/Ctrl+Z теперь работают по одному наведению
после non-canvas выбора; сфокусированный реальный текстовый инпут
(`#wlSearch`) по-прежнему не даёт хоткеям сработать; поведение Escape
(отмена активного инструмента) не изменилось. 198 pytest + оба JS
runtime suites зелёные (добавлен dedicated regression-тест на input-guard,
подтверждён падением при временном отключении guard'а).

## Часть 26 (2026-08-19) — Индикаторные панели: crosshair sync/resize в порядке, drawing tools внутри oscillator pane — реальный баг порчи данных

Последний явно непроверенный пункт из части 24/25. Живая проверка на
проде (QA-аккаунт, RSI как тестовый oscillator-индикатор):

- **Crosshair sync между panes — работает нативно.** Наведение мыши на
  main pane рисует вертикальную crosshair-линию сквозь обе panes сразу,
  RSI-легенда обновляется в такт (подтверждено скриншотами на двух разных
  x-позициях) — это встроенное поведение lightweight-charts' multi-pane
  API (`chart.addSeries(type, opts, paneIndex)`, который проект уже
  использует в `indicators.js`), ни строчки своего кода для этого нет.
- **Resize между panes — работает нативно.** Наведение на границу main/
  RSI pane показывает `row-resize` курсор; реальный mouse-drag на 100px
  меняет `chart.panes()[0].getHeight()`/`[1].getHeight()` ровно на эту
  величину (429→529 / 214→114), опять же встроенная механика библиотеки.
- **НАЙДЕН РЕАЛЬНЫЙ БАГ**: drawing tools внутри oscillator pane не просто
  "не поддерживаются" — они молча портят данные. Весь drawing-движок
  (`coordinateToPriceSafe`/`priceToCoordinateSafe`, `drawings.js`) всегда
  читает/пишет цену через `core.candleSeries` — единственную, main-pane'ную
  price-scale; `_relXY()` меряет координаты от всего `core.container`
  (охватывает все panes разом), а не от pane под курсором. Живой репро:
  вооружить Trend Line и драгнуть внутри RSI pane (y мышью физически в
  RSI-области) — создаётся `trend_line` с ценами вроде 75.33/75.77
  (реальный ценовой диапазон *свечей*, никак не связанный ни с
  RSI-значениями 0-100, ни с тем, куда физически тянул мышь пользователь),
  которая рендерится в main pane в произвольном месте — не в RSI pane, где
  её рисовали, и не там, где она была бы осмысленной.
- **Исправлено** (`b7a94df`): вместо полной pane-aware реализации (свой
  price-scale/render-слой на каждый pane — значительно больший кусок,
  соседний с уже известным "Volume Profile"/"Multiselect" по размеру)
  добавлен guard в `_onPointerDown`/`_touchHitOwnsGesture`: если жест
  начинается ниже `paneHeight(core)` (то есть физически не в main pane) —
  указатель не захватывается, tool остаётся вооружённым, жест проваливается
  к нативному per-pane взаимодействию графика (crosshair/resize там по-
  прежнему работают). To же на touch-пути — иначе `preventDefault` там
  тихо гасил бы нативный pan/pinch в oscillator pane за жест, который всё
  равно не создал бы рисунок.
- **Верифицировано** вживую на проде: тот же RSI-драг (та же geometry, что
  и в репро) теперь не создаёт ничего (`drawings.length: 0`, `activeTool`
  остаётся `"trend_line"` — не сброшен, а именно не начат), драг сразу
  следом внутри main pane создаёт нормальный trend_line с осмысленными
  ценами (78.29/78.85); наведение на RSI pane при вооружённом инструменте
  по-прежнему двигает crosshair и обновляет легенду (не только click, а
  hover) — regressии в нативном взаимодействии нет; консоль чистая.
  198 pytest + оба JS runtime suites зелёные. Regression-тест добавлен
  (`tests/chart_drawing_runtime.test.js`) и подтверждён падением при
  временном откате guard'а (создавался 1 рисунок вместо ожидаемых 0)
  перед восстановлением фикса.
- **Честно не сделано** (следующий кандидат этой же строки ТЗ, крупный):
  полноценное рисование *внутри* oscillator pane — своя price-scale на
  каждый pane, свой render-примитив/hit-test на каждый pane вместо одного
  общего на весь `core`. Реальный TradingView это умеет; здесь — только
  безопасный отказ вместо порчи данных.
