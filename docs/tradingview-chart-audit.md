...
**Частично неверно**: `price_range`/`time_range`/`price_date_range`
инструменты существуют и функциональны (создание, hit-test, рендер,
свойства), но UX не такой, как классический TradingView-Measure
(alt/удержание с live-дельтой цены/%/баров/времени, без создания
персистентного объекта). Текущая реализация всегда создаёт постоянный
объект разметки. **PARTIAL**, не MISSING.
